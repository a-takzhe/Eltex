...
int client_attach(package pack)
{
    ...
    {
        printf("Can't create shm for client %s", pack.message);
        return -1;
    }
    if((sem_id = create_sem(pack.message, PROJ_ID, FOR_READER)) == -1)
    {
        printf("Can't create sem for client %s", pack.message);
